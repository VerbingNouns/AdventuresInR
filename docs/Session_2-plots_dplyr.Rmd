...
```{r}
## LOAD IN DATA ####
dialect_data = read.csv("data/dialectdata.csv")
```
...
## How to automatically save figures to pdf. 
...
```{r}
## LOAD IN DATA ####
rt_data = read.delim("data/reactiondata.txt")
```
...
We don't have a clear example of a continuous dependent variable in this dataset.  Usually, as a linguist, I might be plotting the pitch of someone's speech, or the duration or frequency of someone's vowels.  In the current dataset, we do have a few dependent variables which could be converted into continuous.  Although this may not be exactly ideal, we will use it here as proof of principle.

The syntactic questions from columns 28 to 37 are judgement elicitations of English phrasing, where answers range from *a. I'd say this myself* to *e. No native speaker of English would say this.*.
```{r}
levels(dialect_data$give_it_me)
```

Feasibly, we could convert these to numbers, where 1 represents highly acceptable and 5 represents highly unacceptable.  Let's do this below by creating a new column called `give_it_me_num` for a numeric version.
