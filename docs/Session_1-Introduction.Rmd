...
1. Install the package `pacman`
2. Load the package `tidyverse` using the p_load function.
...
To read .csv files we use the ```read.csv()``` function from base R, o ```read_csv()``` from the tidyverse (I would go with the latter as it also shows you a list of the variable types)
...
(1) Download data for COVID deaths from the World Health Organisation
(2) Download population data from the UN
(3) Calculate COVID infection rate per head of population
(4) Merge this data with World Health Report data.
(5) Is there any relationship between the results of the WHR survey, e.g. GDP per capita, and COVID infection rates?

```{r with errors, eval = FALSE}

whr <- read_csv("WHR_2017.csv")

cov <- read_cvs("WHO-COVID-19-global-data.csv") # Downloaded from; https://covid19.who.int/?gclid=CjwKCAjw8df2BRA3EiwAvfZWaJP1dju-Nt3omCCK7ZtdCtgwoBJOVKD1vC7suVRM1HSVy_0vZHzlDRoCRlkQAvD_BwE

pop <- read_csv("https://population.un.org/wpp/Download/Files/1_Indicators%20(Standard)/CSV_FILES/WPP2019_TotalPopulationBySex.csv")

cov$date <- as.Date(cov$Date_reported) # NB Date_reported is in a weird format. This converts it to a format which is easier to process

whr %>% 
  rename(Country = country) -> # In the "whr" dataset, the variable "country" begins with a lower case C. In order to adequately merge, the key variables need to have the same name
  whr

pop %>%  
  rename(Country = Location) %>% # creates "Country" variable in order to merge data
  filter(Time = 2020 & Variant == "No change") %>% 
  merge(cov) %>% 
  select(date == ("2020-06-01")) %>% 
  mutate(cumulative_cases_per_million = (Cumulative_cases / PopTotal) * 1000000) %>% 
  merge(whr) %>% 
  df


plot(df$cumulative_cases_per_million, df$trust_in_government)

plot(df$cumulative_cases_per_million, df$gdp_per_capita)
...
cov <- read_csv("WHO-COVID-19-global-data.csv") # Downloaded from; https://covid19.who.int/?gclid=CjwKCAjw8df2BRA3EiwAvfZWaJP1dju-Nt3omCCK7ZtdCtgwoBJOVKD1vC7suVRM1HSVy_0vZHzlDRoCRlkQAvD_BwE

pop <- read_csv("https://population.un.org/wpp/Download/Files/1_Indicators%20(Standard)/CSV_FILES/WPP2019_TotalPopulationBySex.csv")

cov$date <- as.Date(cov$Date_reported)
...
R markdown is a special format (.Rmd) which contains both R code, and text. It enables you to produce your report and your analysis as a *single document*.
