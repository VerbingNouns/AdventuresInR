...
aov(mpg ~ hp, data = mtcars %>% filter(cyl == 8))
...
```{r}
g <- ggplot(aes(x = Economy..GDP.per.Capita., y = Happiness.Score), data = df)
g <- g + geom_point()
g <- g + geom_smooth()
g
```
...
mod <- lm(Happiness.Score ~ Economy..GDP.per.Capita., data = df)

summary(mod)

table <- coefficients(summary(mod))

knitr::kable(coefficients(summary(mod)), digits = c(2,2,1,5))
