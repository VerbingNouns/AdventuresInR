...
requiredPackages = c('ggplot2','tidyverse','psycho','lme4','ordinal','lmerTest')
...
## GLM
Let's run a first GLM (Generalised Linear Model). A GLM uses a special family "binomial" as it assumes the outcome has a binomial distribution. To run the results, we will change the reference level for both response and grammaticality. 
...
Logodds can be modified to talk about the odds of an event. For our model above, the odds of "grammatical" receiving a "yes" response increase by 100; whereas receiving a "no" is a mere 0.2.
```{r warning=FALSE, message=FALSE, error=FALSE}
exp(coef(mdl.glm))
```
...
If we had count data, we need to use another family "poisson". 
