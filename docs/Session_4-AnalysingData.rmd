...
EXERCISE - EXPLORE SOME OTHER CORRELATIONS IN THE DATA

## Multiple pairwise comparisons

This is a bit more tricky. The best approach is to create a dataframe containing only those variables that you wish to conduct the pairwise correlations on. The `cor` function from base R creates a correlation matrix. But results are not very publication-read. They can be tidied up via the `kable` package (in `knitr`). There is also the fantastic `corrplot` which will give you a great graphical representation.
...
EXERCISE - Use Google to try to figure out how to run this!
...
NB this odds ratio is far too large, so something has gone wrong with the model.
