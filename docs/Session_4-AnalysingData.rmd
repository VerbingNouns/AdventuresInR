...
requiredPackages = c('ggplot2', 'tidyverse', 'psycho', 'ordinal', 'PresenceAbsence', 'broom', 'emmeans', 'car')
...
Let's subset the data and evaluate the differences between `setosa` and `versicolor` using a t-test and a linear model. We will also check the normality and homogenity of variance in the data

##### Subset

```{r}
irisSub <- subset(iris, Species == c("setosa","versicolor"))
...
```{r}
irisSubSet <- subset(iris, Species == "setosa")
irisSubVers <- subset(iris, Species == "versicolor")
...
```{r}
ggplot(irisSub, aes(x = Sepal.Length))+
  geom_density()+
...
In all cases, the non-statistically significant result indicates that there is no evidence that the variance of two levels of the factor `Species` is statistically significant; i.e., the variances are equal

##### T-test

We then run a t-test on the data. We specify the formula as `y ~ x` and add `var.equal = TRUE`

```{r}
t.test(Sepal.Length ~ Species, data = irisSub, var.equal = TRUE)
```

To interpret the t-test, we say that there is evidence for a statistically significant difference between the two groups: `setosa` and `versicolor`: `t(48) = -6.9334, p < 9.304e-09`. The mean of `setosa` is significantly lower than that of `versicolor`.
...
The results of the linear model are exactly the same, albeit with a difference in the sign of the difference. This indicates that the $\beta$ coefficient for `versicolor` is signifcantly higher that than of `setosa` by `0.8560`: `t(48) = 6.933, p < 9.3e-09`.
