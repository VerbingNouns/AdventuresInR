...
penguins %>% 
  mutate(year_fctr = year %>% as.factor()) -> new_penguins

new_penguins %>% 
  select(-year) -> new_penguins

write_csv(new_penguins, "newpenguins.csv")

# save() # save a complex mode
# load() # load it back in so you don't need to rerun it
```
...
penguins %>% 
  mutate(birdsize = case_when(body_mass_g <  3500 ~ "small", # choose first category to label
                              ...
                              is.na(body_mass_g) ~ as.character(body_mass_g),
...
penguins %>% 
  mutate(widesize = case_when(flipper_length_mm <=  180 ~ "short", 
                              flipper_length_mm > 181 ~ "long",
                              is.na(flipper_length_mm) ~ as.character(flipper_length_mm),
                              TRUE ~ "medium"))
```
...
penguins %>% 
  mutate(wingsize = case_when(flipper_length_mm <=  180 ~ "short", 
                              flipper_length_mm > 180 & flipper_length_mm <= 195 ~ "medium-short",
                              flipper_length_mm > 213 ~ "long",
                              is.na(flipper_length_mm) ~ as.character(flipper_length_mm),
                              TRUE ~ "medium-long"))


list_of_words <- c("cat", "dog", "pear", "banana", "car")
peanut <- "peanut"
cat <- "cat"

cat %in% list_of_words
peanut %in% list_of_words
...
mySurvey %>% 
  pivot_longer(cols = c("q2", "q3", "q5", "q6"), # you can also use col indicies, e.g. 4:9
               names_to = "questions",
...
# fill in plot with colour contingent on species
penguins %>% 
  ggplot(aes(x = bill_length_mm,
             y = bill_depth_mm,
...
penguins %>% 
  ...
  ggplot(aes(x = species,
             y = bill_depth_mm,
             fill = year)) + # add a line to plot colour contingent on a mapping
  theme_bw() +
  geom_boxplot() +
  NULL
...
penguins %>% 
  ggplot(aes(x = bill_length_mm,
             y = bill_depth_mm,
             colour = year)) + # add a line to plot colour contingent on a mapping
  theme_bw() +
  geom_point() +
  NULL
```
